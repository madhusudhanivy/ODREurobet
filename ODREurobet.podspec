#
# Be sure to run `pod lib lint ODREurobet.podspec' to ensure this is a
# valid spec before submitting.
#
# Any lines starting with a # are optional, but their use is encouraged
# To learn more about a Podspec see https://guides.cocoapods.org/syntax/podspec.html
#

Pod::Spec.new do |s|
  s.name             = 'ODREurobet'
  s.version          = '1.0.0'
  s.summary          = 'ODR Implementation'

# This description is used to generate tags and improve search results.
#   * Think: What does it do? Why did you write it? What is the focus?
#   * Try to keep it short, snappy and to the point.
#   * Write the description between the DESC delimiters below.
#   * Finally, don't worry about the indent, CocoaPods strips it!

  s.description      = <<-DESC
For playing inhouse Games
                       DESC

  s.homepage         = 'https://github.com/madhusudhanivy/ODREurobet'
  # s.screenshots     = 'www.example.com/screenshots_1', 'www.example.com/screenshots_2'
  s.license          = { :type => 'MIT', :file => 'LICENSE' }
  s.author           = { 'madhusudhanivy' => 'madhusudhan.putta@ivycomptech.com' }
  s.source           = { :git => 'https://github.com/madhusudhanivy/ODREurobet.git', :tag => s.version.to_s }
  # s.social_media_url = 'https://twitter.com/<TWITTER_USERNAME>'

  s.ios.deployment_target = '11.0'

  s.source_files = 'Source/**/*.{swift, c,h,m,mm,cpp,a,framework,pch}'
  s.swift_version = '5.0'
    s.platforms = {
        "ios": "11.0"
    }
#    /Users/madhusudhan.putta/Eurobet/ReactNative/library/ODREurobet/Example/ODREurobet/Images.xcassets
   s.resource_bundles = {
     'ODREurobet' => ['Source/**/*.xcassets', 'Source/**/*.xib']
   }

  # s.public_header_files = 'Pod/Classes/**/*.h'
  # s.frameworks = 'UIKit', 'MapKit'
  # s.dependency 'AFNetworking', '~> 2.3'
  
  s.dependency 'SSZipArchive'
  s.dependency 'GCDWebServer', '3.5.3'
#  s.dependency 'React-Core', :path => '../node_modules/react-native/'

  
end
